...
void window(){
    ...
    gotoxy(20,3);
    printf("Dawood University Of Engineering And Technology");
    gotoxy(31,4);
        SetColor(17);	}
...
int conf_record(char id[]){
   ...
   //new record is already in the database.
    ...
    if(fp == NULL){
        ...
        fflush(stdin);
        gotoxy(print,10);printf("ID: ");gets(emp.ID);
        //here you can confirms the ID
        gotoxy(print,12);printf("Name: ");gets(emp.name);
        gotoxy(print,14);printf("Address: ");gets(emp.add);
        gotoxy(print,16);printf("Parent's name: ");gets(emp.parname);
        gotoxy(print,18);printf("Salary: ");scanf("%d",&emp.Salary);
        gotoxy(print,20);printf("Phone Number: ");scanf("%ld",&emp.phone_no);
...
void search_employee(){
    ...
    if(isFound == 1){
        ...
        gotoxy(37,14);printf("ID: %s",emp.ID);
        gotoxy(37,15);printf("Name: %s",emp.name);
        gotoxy(37,16);printf("Address: %s",emp.add);
        gotoxy(37,17);printf("Parent's Name: %s",emp.parname);
        gotoxy(37,18);printf("Salary: %d",emp.Salary);
        gotoxy(37,19);printf("Phone No: %ld",emp.phone_no);
    }else{
...
void mod_employee(){
    ...
    while(fread(&emp, sizeof(emp),1,fp) == 1){
        if(strcmp(s_id, emp.ID) == 0){
            fflush(stdin);
            gotoxy(print,12);printf("ID: ");gets(emp.ID);
            gotoxy(print,13);printf("Name: ");gets(emp.name);
            gotoxy(print,14);printf("Address: ");gets(emp.add);
            gotoxy(print,15);printf("Parent's name: ");gets(emp.parname);
            gotoxy(print,16);printf("Salary: ");scanf("%d",&emp.Salary);
            gotoxy(print,17);printf("Phone Number: ");scanf("%ld",&emp.phone_no);
...
void main_window(){
    ...
    int x = 2;
    while(1){
        gotoxy(x,8);printf("1. Add employee");
        gotoxy(x,10);printf("2. Search employee");
        gotoxy(x,12);printf("3. Modify employee Record");
        gotoxy(x,14);printf("4. Generate promotion slip");
        gotoxy(x,16);printf("5. Delete employee Record");
...
int main(){
    ...
    SetConsoleTitle("Group1- Employee Record System");
    window();
    //use_pass_field();
    main_window();
    return 0;	}
